...
import { addConsumedFood, clearConsumedFoodsByMeal, getTodaysConsumedFoods, getUserProfile, removeConsumedFood, updateDailySummary, addPoints, getDailySummary, getTodayDateString } from '../services/saudeService';
...
export const useReceitasViewModel = () => {
  ...
  const route = useRoute<RouteProp<RootStackParamList, 'Receitas'>>();
    ...
    useCallback(() => {
      const loadData = async () => {
        ...
        try {
          ...
          const foods = alimentosData.map((item: any) => ({
            id: item.id.toString(),
  ...
  const displayedFoods = useMemo(() => {
    if (!searchText) {
      return allFoods.slice(0, 50);
    }
    return allFoods.filter(f => f.name.toLowerCase().includes(searchText.toLowerCase())).slice(0, 50);
  }, [allFoods, searchText]);
  ...
  useEffect(() => {
    const handleCalorieUpdate = async () => {
      if (!userProfile || isLoading || !userProfile.daily_calorie_goal) return;

      const today = getTodayDateString();
      const calorieGoal = userProfile.daily_calorie_goal;

      // Pega o resumo ANTES da atualização para saber o estado anterior
      const summaryBeforeUpdate = await getDailySummary(today);
      const goalAlreadyMet = summaryBeforeUpdate.caloriesConsumed >= calorieGoal;

      // Atualiza as calorias no banco de dados
      await updateDailySummary(userProfile.id, { calories_consumed: totalConsumedKcal });

      // Verifica se a meta foi atingida NESTA atualização
      const goalMetNow = totalConsumedKcal >= calorieGoal;

      if (goalMetNow && !goalAlreadyMet) {
        console.log('[useReceitasViewModel] Meta de calorias atingida! Adicionando 10 pontos.');
        await addPoints(userProfile.id, 10);
      }
    };

    handleCalorieUpdate().catch(e => console.error('[useReceitasViewModel] Erro ao atualizar calorias e pontos:', e));

...
import { Receita, useReceitasSupabaseViewModel } from '../viewmodels/useReceitasSupabaseViewModel';

// Tipagem da rota
type ReceitasListScreenRouteProp = RouteProp<RootStackParamList, 'ReceitasList'>;
...
const ReceitasListScreen = () => {
  ...
  const route = useRoute<ReceitasListScreenRouteProp>();
  const { categoria } = route.params;

  const { receitas, isLoading, favoriteIds, fetchReceitasPorCategoria, toggleFavorite } = useReceitasSupabaseViewModel();

  useEffect(() => {
    if (categoria) {
      fetchReceitasPorCategoria(categoria);
    }
  }, [categoria]);

  const handleRecipePress = (receitaId: number) => {
    // @ts-ignore - Navegando para uma rota que será adicionada
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>Nenhuma receita encontrada para esta categoria.</Text>}
      />
    </SafeAreaView>
  );
};
